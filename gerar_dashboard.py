  ...
  <script>
    ...
    function obterResumoTecnicosPainel() {{
      ...
      tecnicoHistoryData.forEach((registro) => {{
        ...
        item.novasOs += Number(registro.novas_os_no_periodo || 0);
        item.transferenciasRecebidas += Number(registro.transferencias_entrada_no_periodo || 0);
        item.reabertas += Number(registro.reabertas_no_periodo || 0);
        item.encerradasDaCarteira += Number(registro.encerradas_da_carteira_no_periodo || 0);
        item.transferenciasCedidas += Number(registro.transferencias_saida_no_periodo || 0);
    ...
    function renderResumoTecnicosPainel() {{
      ...
      itens.forEach((item) => {{
        ...
        const detalhes = [
          ["OS iniciais", item.osIniciais],
          ["Entradas", item.entradas],
          ["Novas O.S.", item.novasOs],
          ["Transferências recebidas", item.transferenciasRecebidas],
          ["Reabertas", item.reabertas],
          ["Saídas", item.saidas],
          ["Encerradas da carteira", item.encerradasDaCarteira],
          ["Transferências cedidas", item.transferenciasCedidas],
        ]
          .filter(([, valor]) => Number(valor) !== 0)
          .map(([rotulo, valor]) => `<span class="metric-sub">${{rotulo}}: ${{valor}}</span>`)
          .join("");
        card.innerHTML = `
      ...
      resumoTecnicosMeta.textContent = `Mostrando ${{itens.length}} técnico(s) do grupo Técnicos com O.S. iniciais, total atual, entradas e saídas discriminadas no intervalo selecionado.`;
    }}
