    ...
    function obterChavesReincidentes(registros) {{
      ...
      options: {{
	    ...
	    function obterLinhasTooltipHistoricoTecnicos(indice) {{
	      ...
	      if (resumoHistoricoTecnicosAtual.modo === "multiplos") {{
	        return {{
	    ...
	    function renderTooltipHistoricoTecnicos(context) {{
	      ...
	      const conteudo = resumoHistoricoTecnicosAtual?.modo === "multiplos"
	        ? {{
	            titulo: resumoHistoricoTecnicosAtual?.labels?.[indice] || "",
	            linhas: tooltip.dataPoints.map((ponto) => `${{ponto.dataset.label}}: ${{ponto.formattedValue}} O.S.`),
	          }}
      ...
      function filtroHistoricoTecnicosCorresponde(linha) {{
	    ...
	    function renderHistoricoTecnicosEventos() {{
	      ...
	      if (resumo.modo === "multiplos") {{
	        historicoTecnicosEventosMeta.textContent = "Selecione um técnico no filtro de usuário para ver o detalhamento de eventos por coleta.";
	        historicoTecnicosEventosBody.innerHTML = `<tr><td colspan="8" class="empty">O gráfico acima mostra a evolução de cada técnico; selecione um usuário para detalhar os eventos.</td></tr>`;
	        return;
	      }}
	      const linhas = [];
	      let totalEventos = 0;
	    ...
	    function renderGraficoHistoricoTecnicos() {{
	      if (!filtroUsuario.value) {{
	        const resumoMultiplos = agruparResumoDiario();
	        resumoHistoricoTecnicosAtual = {{
	          modo: "multiplos",
	          labels: resumoMultiplos.labels,
	          totalCapturas: resumoMultiplos.totalCapturas,
	          totalCapturasOriginais: resumoMultiplos.totalCapturasOriginais,
	          totalMudancas: resumoMultiplos.totalMudancas,
	          totalTecnicos: resumoMultiplos.totalTecnicos,
	          intervalo: resumoMultiplos.intervalo,
	        }};
	        if (historicoTecnicosIndiceSelecionado >= resumoMultiplos.labels.length) {{
	          historicoTecnicosIndiceSelecionado = resumoMultiplos.labels.length - 1;
	        }}
	        graficoHistoricoTecnicos.data.labels = resumoMultiplos.labels;
	        graficoHistoricoTecnicos.data.datasets = resumoMultiplos.datasets.map((dataset) => ({{
	          ...dataset,
	          label: dataset.label.replace(/^Itinerário - /, ""),
	        }}));
	        graficoHistoricoTecnicos.options.plugins.legend.display = resumoMultiplos.datasets.length > 1;
	        ...
	        if (!resumoMultiplos.labels.length) {{
	          historicoTecnicosMeta.textContent = "Sem histórico suficiente para montar a evolução dos técnicos no intervalo selecionado.";
	          renderHistoricoTecnicosEventos();
	          return;
	        }}

	        historicoTecnicosMeta.textContent = `Histórico de ${{resumoMultiplos.totalTecnicos}} técnico(s) no intervalo selecionado, com ${{resumoMultiplos.totalMudancas}} marcação(ões) distribuídas em ${{resumoMultiplos.totalCapturas}} coleta(s) destacadas de ${{resumoMultiplos.totalCapturasOriginais || resumoMultiplos.totalCapturas}}. Use o filtro de usuário para detalhar um técnico específico.`;
	        renderHistoricoTecnicosEventos();
	        return;
	      }}
	      ...
	      graficoHistoricoTecnicos.options.plugins.legend.display = false;
