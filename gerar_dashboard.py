	            ...
	            callbacks: {{
	              title(items) {{
	                ...
	                const evento = Array.isArray(resumoHistoricoTecnicosAtual.eventosCarteira)
	                  ...
	                  : null;
	                return evento?.capturadoEm || items[0].label || "";
	              }},
	              label(item) {{
	                if (!resumoHistoricoTecnicosAtual) return "";
	                const indice = item.dataIndex;
	                const valorReal = resumoHistoricoTecnicosAtual?.totalCarteira?.[indice];
	                return `Itinerário: ${{valorReal ?? item.formattedValue}} O.S.`;
	              }},
	              afterBody(items) {{
	                if (!items.length || !resumoHistoricoTecnicosAtual) return [];
	                const indice = items[0].dataIndex;
