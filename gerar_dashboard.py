      ...
      graficoMensal.data.datasets[3].data = resumo.map((item) => item.inviabilidade);
	    ...
	    function calcularEscalaHistoricoTecnicos(valores) {{
	      const numeros = Array.isArray(valores)
	        ...
	        : [];
	      if (!numeros.length) {{
	        return {{ beginAtZero: true, min: 0, max: 5 }};
	      }}
	    ...
	    function renderGraficoHistoricoTecnicos() {{
	      ...
	      historicoTecnicosMeta.textContent = `Histórico de ${{resumo.tecnicoSelecionado}} com ${{resumo.totalCapturas}} coleta(s) no intervalo selecionado; o gráfico mostra como o itinerário variou no período e terminou com ${{itinerarioFinal}} O.S.${{complementoLacunas}}`;
	      renderHistoricoTecnicosEventos();
	    }}
